...
// Imperative API - can be used outside of React components
let toastRef: ToastContextType | null = null;

export const toast = {
  show: (options: Omit<ToastProps, 'id' | 'onDismiss'>) => {
    if (!toastRef) {
      console.warn('ToastProvider not initialized yet');
      return '';
    }
    return toastRef.showToast(options);
  },
  success: (title: string, message?: string, options?: Omit<ToastOptions, 'title' | 'message'>) => {
    if (!toastRef) {
      console.warn('ToastProvider not initialized yet');
      return '';
    }
    return toastRef.showSuccess(title, message, options);
  },
  error: (title: string, message?: string, options?: Omit<ToastOptions, 'title' | 'message'>) => {
    if (!toastRef) {
      console.warn('ToastProvider not initialized yet');
      return '';
    }
    return toastRef.showError(title, message, options);
  },
  warning: (title: string, message?: string, options?: Omit<ToastOptions, 'title' | 'message'>) => {
    if (!toastRef) {
      console.warn('ToastProvider not initialized yet');
      return '';
    }
    return toastRef.showWarning(title, message, options);
  },
  info: (title: string, message?: string, options?: Omit<ToastOptions, 'title' | 'message'>) => {
    if (!toastRef) {
      console.warn('ToastProvider not initialized yet');
      return '';
    }
    return toastRef.showInfo(title, message, options);
  },
  update: (id: string, updates: { title?: string; message?: string }) => {
    if (!toastRef) {
      console.warn('ToastProvider not initialized yet');
      return;
    }
    toastRef.updateToast(id, updates);
  },
  dismiss: (id: string) => {
    if (!toastRef) {
      console.warn('ToastProvider not initialized yet');
...
export const ToastProvider: React.FC<ToastProviderProps> = ({
  ...
  // Set the imperative API reference
  useEffect(() => {
