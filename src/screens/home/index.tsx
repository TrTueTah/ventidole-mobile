...
export default function HomeScreen() {
  ...
  const handleSwap = () => {
    showSuccess(
      t('SWAP_SUCCESS'),
      t('SWAP_DETAILS', { amount: ~~(Math.random() * 1000) }),
      {
        position: 'top',
      },
    );
      ...
      <ScrollView className={'flex-1'}>
        ...
        <View className={'p-8 pt-safe-offset-4 flex-row justify-between'}>
          ...
          <View className={'flex flex-row gap-2'}>
            <Pressable
              className={
                'bg-background border border-neutrals900 w-16 h-16 rounded-full justify-center items-center'
              }
            >
